...
def naiveSoftmaxLossAndGradient(
    ...
    softmax_array = np.dot(outsideVectors, centerWordVec)
    # print("hello!!!!!!!!!!!")
    # print(softmax_array.shape)
    tmp = softmax(softmax_array)
    # print(tmp.shape)
    ...
    gradOutsideVecs = centerWordVec * tmp.reshape(-1,1)
...
def negSamplingLossAndGradient(
    ...
    a = sigmoid(np.dot(outsideVectors[negSampleWordIndices], centerWordVec))
    b = sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec))
    loss = -np.log(b) - np.sum(np.log(1-a))

    gradCenterVec = (b - 1) * outsideVectors[outsideWordIdx] + np.sum(a.reshape(-1,1) * outsideVectors[negSampleWordIndices], axis = 0)
    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx] = (b - 1) * centerWordVec
    for i, j in enumerate(negSampleWordIndices):
        gradOutsideVecs[j] += a[i] * centerWordVec
    ### END YOUR CODE
