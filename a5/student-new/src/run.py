...
if args.function == 'pretrain':
    ...
    #     num_workers=4
    traincon = trainer.TrainerConfig(max_epochs=650,
                                     batch_size=128,
                                     learning_rate=6e-3,
                                     lr_decay=True,
                                     warmup_tokens=512*20,
    ...
    tr = trainer.Trainer(model, pretrain_dataset, None, traincon)
    tr.train()
    # 把模型参数储存起来，方便接下来finetuning用
    ...
    traincon = trainer.TrainerConfig(max_epochs=10,
                                     batch_size=256,
                                     learning_rate=6e-4,
                                     lr_decay=True,
                                     warmup_tokens=512*20,
    ...
    tr = trainer.Trainer(model, train_dataset, None, traincon)
    tr.train()
    torch.save(model.state_dict(), args.writing_params_path)
    # raise NotImplementedError
